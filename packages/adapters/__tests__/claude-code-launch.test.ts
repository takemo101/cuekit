...
describe("buildClaudeCodeLaunchCommand", () => {
	it("defaults to the 'claude' binary", () => {
		expect(buildClaudeCodeLaunchCommand(spec())).toBe("claude 'do the thing'");
	});

	it("respects a custom claudeBin", () => {
		expect(buildClaudeCodeLaunchCommand(spec(), { claudeBin: "/usr/local/bin/claude" })).toBe(
			"/usr/local/bin/claude 'do the thing'",
		);
	});
	...
	it("shell-quotes objectives with single quotes via POSIX '\\'' escape", () => {
		...
		expect(out).toBe("claude 'it'\\''s a test'");
	});
	...
	it("shell-quotes objectives with shell metacharacters", () => {
		...
		expect(out).toBe("claude 'rm -rf $HOME; echo pwned'");
	});
