...
- `native_task_ref` under the v0 tmux pane backend stores the tmux `pane_id` of the child (see adapter spec Section 3.7). The tmux session name (`cuekit-{session_id}`) and window name (`task-{task_id_short}`) are derivable from `session_id` and `id`, so they do not need their own columns.
