...
- `agent_kind` — default runtime adapter, e.g. `claude-code`, `opencode`, `jcode`, `gemini`, or `pi`.
...
| `agent_kind` | string | optional | optional | — | Runtime adapter, e.g., `claude-code`, `opencode`, `jcode`, `gemini`, `pi`. If omitted, resolved value comes from lower scope. |
