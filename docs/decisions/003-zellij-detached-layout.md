...
Task rows store the backend kind in `native_task_ref` (`tmux:%1`, `zellij:ct-<task_id>/pane`). Legacy unqualified refs (`%1`) are treated as tmux. If a long-lived process was started with one backend and the project config later switches to another, status polling must not infer pane death through the wrong backend. In that mismatch case, cuekit leaves the task non-terminal and suppresses steering/liveness actions from the stale adapter view rather than fabricating a failed result. Attach remains available for known backends because the command can be reconstructed from the stored backend kind and deterministic session name (`tmux attach-session -t cuekit-task-<task_id>` or `zellij attach ct-<task_id>`). See the operator smoke guide at [`docs/guides/multiplexer-backends.md`](../guides/multiplexer-backends.md).
