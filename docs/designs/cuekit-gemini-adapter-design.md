...
- `buildGeminiLaunchCommand({ agent_kind: "gemini", objective: "x" })` defaults to `gemini -y '<prompt>'`.
- Adding `model: "gemini-2.5-flash"` produces `gemini -y -m 'gemini-2.5-flash' '<prompt>'`.
- Adding `adapter_options.mode: "batch"` produces `gemini -y -m '<model>' -p '<prompt>'`.
- Adding `adapter_options.dangerously_skip_permissions: false` omits `-y`.
