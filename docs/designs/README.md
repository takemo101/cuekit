...
- [`cuekit-hooks-design.md`](cuekit-hooks-design.md) — fire-and-forget shell hooks for task/team lifecycle events, configured in `.cuekit.yaml`.
...
- [`cuekit-swarm-lite-design.md`](cuekit-swarm-lite-design.md) — small parent-led team coordination design centered on team snapshots, minimal blackboard events, cooperative profiles, and manual steering without becoming a swarm runtime.
...
- [`cuekit-ai-ergonomics-design.md`](cuekit-ai-ergonomics-design.md) — umbrella design for lowering per-call cost, migrating critical coordinator behaviors from prompt to API, adding safety nets (quotas, idempotency, auto-cleanup), and exposing self-observability metrics so AI agents can reach for cuekit frequently and reliably.
...
- [`cuekit-herdr-multiplexer-backend-design.md`](cuekit-herdr-multiplexer-backend-design.md) — proposed `herdr` backend mapping for cuekit tasks/teams onto Herdr sessions, workspaces, tabs, and panes.
