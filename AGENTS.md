...
- **Multiplexer は tmux 既定 / zellij / herdr 任意**: `.cuekit.yaml` の `multiplexer.backend` で切り替え可能。zellij ≥ 0.43、herdr ≥ 0.5。いずれも無い場合は `logger.warn` を出して tmux にソフトフォールバック (`multiplexer.strict: true` でハードフェイル)。backend 切替時は `native_task_ref` の backend kind を正とし、attach は維持するが steer/cancel/cleanup は owning backend 以外では抑制する。 → [multiplexer design](docs/designs/cuekit-multiplexer-backend-design.md) / [Herdr design](docs/designs/cuekit-herdr-multiplexer-backend-design.md) / [zellij ADR 003](docs/decisions/003-zellij-detached-layout.md) / [guide](docs/guides/multiplexer-backends.md)
...
- **Multiplexer backend 切替**: `.cuekit.yaml` の `backend` を変更しても、既存タスクは spawn 時の owning backend のまま。steer/cancel/delete は `backendMismatchError` で拒否。attach は `native_task_ref` から復元される。 → [multiplexer backend design](docs/designs/cuekit-multiplexer-backend-design.md)
